...
namespace Web.Controllers
{
    ...
    public class UsersController : Controller, IFullRestApiController<string, UserPostViewModel, UserPutViewModel>
    {
        ...
        public async Task<IActionResult> GetAsync(GetFilterViewModel filter)
        {
            ...
            List<ApplicationUser> users = await _userService.GetListAsync(filter.Count, filter.Offset);
            return Ok(ToViewModelsAsync(users));
        }
        ...
        public async Task<IActionResult> GetAsync(string id)
        {
            ...
            return Ok(ToViewModelAsync(entity));
        }
        ...
        public async Task<IActionResult> PutAsync([FromBody]UserPutViewModel putViewModel)
        {
            ...
            if (result.Succeeded)
            {
                return Ok(ToViewModelAsync(user));
            }
        ...
        public async Task<IActionResult> PostAsync([FromBody]UserPostViewModel postViewModel)
        {
            ...
            if (result.Succeeded)
            {
                var viewModel = ToViewModelAsync(user);
        ...
        public async Task<IActionResult> SendResetPasswordCodeAsync(string email)
        {
            ...
            await _emailService.SendEmailAsync(user.Email,
                "Reset Password",
                $"Your reset password code: {code}", ConfigureService.MailOptions);
        ...
        public async Task<IActionResult> SendConfirmEmailCodeAsync()
        {
            ...
            await _emailService.SendEmailAsync(user.Email,
                "Confirm your email",
                $"Confirm email by clicking on the link: <a href='{callbackUrl}'>link</a>", ConfigureService.MailOptions);
        ...
        private async Task<List<UserGetViewModel>> ToViewModelsAsync(IEnumerable<ApplicationUser> users)
        {
            ...
            {
                result.Add(await ToViewModelAsync(user));
            }

            return result;
        }

        private async Task<UserGetViewModel> ToViewModelAsync(ApplicationUser user)
        {
